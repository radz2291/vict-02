...
const CONSUMERS = { compat: 5301, direct: 5302 };
...
async function driveConsumer(variant, port) {
  ...
  // P5: frozen renderer identity + facade/direct binding identity from the
  // in-page probe (see App.svelte) — same bindings, no second implementation.
  const identity = await page.evaluate(() => window.__p5identity);
  if (variant === 'compat') {
    check(
      `${variant}: facade re-exports the SAME runtime bindings as ui-svelte (createVictRenderer/renderVictApplication/VitApp/identity constants)`,
      identity.sameCreate === true && identity.sameRender === true && identity.sameVitApp === true &&
        identity.sameIdConst === true && identity.sameRevConst === true,
      JSON.stringify(identity),
    );
  }
  const identityOk =
    identity.id === 'renderer.svelte-kit' &&
    identity.revision === '5.0.0' &&
    (variant === 'compat'
      ? identity.sameIdConst === true && identity.sameRevConst === true
      : identity.idConst === 'renderer.svelte-kit' && identity.revConst === '5.0.0') &&
    Array.isArray(identity.roles) &&
    identity.roles.includes('table') && identity.roles.includes('conversation') && identity.roles.includes('tabs');
  check(
    `${variant}: frozen renderer identity through the packed path (id/revision/roles)`,
    identityOk,
    JSON.stringify(identity),
  );
...
try {
  for (const variant of Object.keys(CONSUMERS)) {
    ...
    // P5: exactly ONE renderer implementation in the built bundle — the
    // identity string and the host marker each occur exactly once in ALL JS.
    const jsFiles = assets.filter((a) => a.endsWith('.js'));
    const js = jsFiles.map((f) => readFileSync(join(dest, 'dist', 'assets', f), 'utf8')).join('\n');
    const idCount = js.split('renderer.svelte-kit').length - 1;
    const hostCount = js.split('vict-host').length - 1;
    check(
      `${variant}: built bundle contains EXACTLY ONE renderer implementation (identity x${idCount}, host marker x${hostCount})`,
      idCount === 1 && hostCount === 1,
