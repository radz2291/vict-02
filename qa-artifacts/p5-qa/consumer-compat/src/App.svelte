<script>
  ...
  // P5 QA identity probe: the facade must re-export the SAME bindings as the
  // direct implementation package (same function reference, same constants).
  import * as compatNS from '@victframework/renderer-svelte';
  import * as directNS from '@victframework/ui-svelte';
  const compatRenderer = compatNS.createVictRenderer();
  window.__p5identity = {
    sameCreate: compatNS.createVictRenderer === directNS.createVictRenderer,
    sameRender: compatNS.renderVictApplication === directNS.renderVictApplication,
    sameVitApp: compatNS.VitApp === directNS.VitApp,
    sameIdConst: compatNS.RENDERER_ID === directNS.RENDERER_ID,
    sameRevConst: compatNS.RENDERER_REVISION === directNS.RENDERER_REVISION,
    id: compatRenderer.id,
    revision: compatRenderer.revision,
    roles: [...compatRenderer.supportedSurfaceRoles],
  };

  const registry = createComponentRegistry('registry.consumer.p5.compat', '1');
