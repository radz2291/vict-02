...
<figure class="vict-figure" data-surface={surfaceId} role="img" aria-label={summary}>
  {#if title !== undefined}<figcaption>{title}</figcaption>{/if}
  <svg class="vict-chart" viewBox="0 0 {WIDTH} {HEIGHT}" aria-hidden="true" data-testid="chart-svg">
    ...
    {:else}
      ...
      {#each points as point, index (point.label)}
        ...
        <text x={x} y={HEIGHT - PAD.bottom + 16} text-anchor="middle">{point.label}</text>
      {/each}
    {/if}
  </svg>
