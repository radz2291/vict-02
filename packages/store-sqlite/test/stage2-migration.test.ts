...
describe('stage 02 database migrates forward to the Stage 03 schema', () => {
  it('migrates real Stage 02 data without identity, event, or activation loss', { timeout: 120_000 }, async () => {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          const runs = handle.prepare('SELECT run_id, status, steps, record_revision FROM vict_run ORDER BY run_id;').all() as unknown as {
            run_id: string;
            status: string;
          ...
          const events = handle.prepare('SELECT run_id, seq FROM vict_run_event ORDER BY run_id, seq;').all() as unknown as {
            run_id: string;
            seq: number;
          }[];
          ...
          const activations = handle.prepare('SELECT COUNT(*) AS c FROM vict_activation;').get() as { c: number };
