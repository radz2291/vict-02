...
function slsaPayload() {
  const base = {
    ...
    subject: [
      {
        name: 'pkg:npm/%40victframework/contracts@0.3.0-rc.1',
        digest: {
...
describe('BOUND_CANDIDATE', () => {
  it('is exactly the amendment-bound recovery identity', () => {
    expect(BOUND_CANDIDATE.version).toBe('0.3.0-rc.1');
    expect(BOUND_CANDIDATE.sourceSha).toBe('a98dd015a3cb6f8e210447dcc89f5cdefab02ec9');
    expect(BOUND_CANDIDATE.originalRunId).toBe(35530894104);
    expect(BOUND_CANDIDATE.packageCount).toBe(13);
    expect(BOUND_CANDIDATE.registry).toBe('https://registry.npmjs.org/');
    expect(BOUND_CANDIDATE.correctedEngineSha).toBe('8844f54e0a6d59f1d81f241eb7ce32d3bad21e8c');
    expect(BOUND_CANDIDATE.candidateTag).toBe('vict-0.3.0-rc');
    expect(BOUND_CANDIDATE.expectedLatest).toBe('0.2.0');
    expect(BOUND_CANDIDATE.forbiddenStableVersion).toBe('0.3.0');
  });
...
describe('evaluateRegistryMemberState', () => {
  const good = () => ({
    versions: {
      '0.2.0': { dist: { integrity: 'sha512-AAA=' } },
      '0.3.0-rc.1': { dist: { integrity: GOOD_INTEGRITY } },
    },
    'dist-tags': { latest: '0.2.0', 'vict-0.3.0-rc': '0.3.0-rc.1' },
  });
  ...
  it('negative control: a missing candidate version fails', () => {
    ...
    delete packument.versions['0.3.0-rc.1'];
  ...
  it('negative control: a published stable 0.3.0 fails', () => {
    const packument = good();
    packument.versions['0.3.0'] = { dist: { integrity: 'sha512-BBB=' } };
    const verdict = evaluateRegistryMemberState('@victframework/contracts', packument);
    expect(verdict.ok).toBe(false);
    expect(verdict.problems.join(' ')).toContain('stable must remain unpublished');
  });

  it('negative control: latest moved off 0.2.0 fails', () => {
    const packument = good();
    packument['dist-tags'].latest = '0.3.0-rc.1';
    const verdict = evaluateRegistryMemberState('@victframework/contracts', packument);
    expect(verdict.ok).toBe(false);
    expect(verdict.problems.join(' ')).toContain("'latest' is '0.3.0-rc.1'");
  });

  it('negative control: a moved or missing candidate tag fails', () => {
    const moved = good();
    moved['dist-tags']['vict-0.3.0-rc'] = '0.2.0';
    expect(evaluateRegistryMemberState('@victframework/contracts', moved).ok).toBe(false);
    const missing = good();
    delete missing['dist-tags']['vict-0.3.0-rc'];
  ...
  it('negative control: missing registry integrity fails', () => {
    const packument = good();
    packument.versions['0.3.0-rc.1'].dist = {};
...
describe('evaluateSlsaProvenance', () => {
  ...
  it('accepts the observed live provenance shape with all five bindings', () => {
    ...
    expect(verdict.identities.invocationId).toContain('/35530894104/');
...
describe('evaluateRegistryManifest', () => {
  ...
  it('negative control: a range pin on an internal member fails', () => {
    const verdict = evaluateRegistryManifest(
      manifest({ '@victframework/runtime': '^0.3.0-rc.1' }),
      FROZEN_PUBLISH_ORDER,
    );
    expect(verdict.ok).toBe(false);
    expect(verdict.problems.join(' ')).toContain("pinned '^0.3.0-rc.1'");
  });

  it('negative control: a wrong exact pin fails', () => {
    const verdict = evaluateRegistryManifest(
      manifest({ '@victframework/runtime': '0.2.0' }),
      FROZEN_PUBLISH_ORDER,
    );
    expect(verdict.ok).toBe(false);
    expect(verdict.problems.join(' ')).toContain("pinned '0.2.0'");
  });
  ...
  it('negative control: a manifest at a different version fails', () => {
    const verdict = evaluateRegistryManifest(
      { name: '@victframework/server', version: '0.3.0-rc.2', dependencies: {} },
      FROZEN_PUBLISH_ORDER,
    );
    expect(verdict.ok).toBe(false);
...
describe('auditEvidenceWorkflow negative controls', () => {
  ...
  it('refuses a registry-mutating npm command', () => {
    const mutated =
      base() +
      '\n      - name: smuggled\n        run: npm deprecate @victframework/sdk@0.3.0-rc.1 --message x\n';
    const audit = auditEvidenceWorkflow(mutated);
    expect(audit.ok).toBe(false);
  });
  ...
  it('refuses mutated dispatch defaults (the recovery is bound)', () => {
    const mutated = base().replace("default: '0.3.0-rc.1'", "default: '0.3.0-rc.2'");
    const audit = auditEvidenceWorkflow(mutated);
    expect(audit.ok).toBe(false);
