...
(`docs/builder-kit/capability-catalog.json`), produced from the **typed
authoring declarations** themselves (the Stage 04 closed capability
contracts and immutable authoring captures: contract ID/revision, declared
effect class, authority/permission requirements, idempotency, and the
author-declared summary). Each entry records its source package, declaring
module, and declaration-content digest. A fresh agent discovers
capabilities through the catalog and follows its references to the
declarations for detail. The catalog is a recorded input of the base pack,
so the freshness gate enforces it: a capability present in code but
missing, stale, or dangling in the catalog fails verification (§4.3) —
typed declarations are the source, the catalog is the generated view, and
the pack carries the view with provenance. No hand-written capability
